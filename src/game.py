...
class GomokuGame:
    ...
    def is_valid(self, row, col):
        ...
        ) and position_is_empty(self.game, row, col)
    ...
    def draw_info(self):
        ...
        text_rect = text_surface.get_rect(
            center=(self.window_size // 2, self.window_size + INFO_HEIGHT // 2)
        )
        ...
        if self.game_over:
            restart_text = FONT_SMALL.render(
                "Drücke eine beliebige Taste für Neustart", True, WHITE
            )
            restart_rect = restart_text.get_rect(
                center=(self.window_size // 2, self.window_size + INFO_HEIGHT - 15)
            )
    ...
    def process_bot_move(self):
        ...
        if self.bot_mode == "ai":
            ...
            row, col = generate_next_move_random(self.game)
            make_move(self.game, row, col, self.current_player)
    ...
    def run(self):
        ...
        while running:
            ...
            if (
                not self.game_over
                and not self.two_player
                and self.current_player == PLAYER_BLACK
            ):
...
if __name__ == "__main__":
    ...
    game = GomokuGame(
        board_size=args.size, bot_mode=args.bot, two_player=two_player_mode
    )
    game.run()
